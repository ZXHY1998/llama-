...
import time
from model_hook import *  # 从nodel_hook.Py文件中加载自定义的函数
...
def get_ds_model(args, dataloader_dict):
    ...
    log_dist(
        f"Finally total_params: {total_params} trainable_params: {trainable_params} ratio {trainable_params / total_params if total_params > 0 else -1:.4 %} ")
    # 获取自定义的优化器和学习率调度器
    ...
    if op_lr_dict is None:
        ...
        lr_scheduler = op_lr_dict.get("Ir_scheduler", None)
        optimizer = op_lr_dict.get("optimizer", None)
    # 初始化deepspeed
    model, _, _, Ir_scheduler = deepspeed.initialize(
        model = model,
...
def save_hf_format(model, tokenizer, args, sub_folder=""):
    """
    保存模型为HuggingFace格式，以便后续可以用hf.trom_pretrained加载
    """
    ...
    print('==============================')
    print(f'Model saved at: toutput_dir｝')
    print('==============================')
...
def main():
    ...
    for epoch in range(0, args.epochs):
        ...
        train_dataloader = dataloader_dict["train _dataloader"]
        st = time.time()
        num_total_steps = len(train_dataloader)
        for step, batch in enumerate(train_dataloader):
            batch = {k: v.to(device) for k, v in batch.items()}  # #batch中的数据转移到device上
            ...
            if step>0 and args.save_steps > 0 and step % args.save_steps == 0:
                ...
                if args.global_rank == 0:
                    save_hf_format(
                    model, dataloader_dict['tokenizer'], args, sub_folder = f' epoch{epoch}_step-{step}-hf')
